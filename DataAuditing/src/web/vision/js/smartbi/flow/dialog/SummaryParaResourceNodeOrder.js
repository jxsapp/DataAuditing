...
var SummaryParaResourceNodeOrder = function(parent) {
	//SummaryParaResourceNodeOrder.superclass.constructor.call(parent);
...
lang.extend(SummaryParaResourceNodeOrder, ResourceNodeOrder);

SummaryParaResourceNodeOrder.prototype.destroy = function() {
	this.grid.destroy();
}

//初始化
//节点信息最少应该包含 名称 别名 ID 类型
SummaryParaResourceNodeOrder.prototype.init = function(nodes) {
	this.grid.init(nodes);
}

SummaryParaResourceNodeOrder.prototype.getOrderNodes = function() {
	return this.grid.getOrder();
}


SummaryParaResourceNodeOrder.prototype.isDirty = function() {
	return this.grid.isDirty;
}

//插入节点
SummaryParaResourceNodeOrder.prototype.addOneNode = function(node) {
	this.grid.addOneNode(node);
}

SummaryParaResourceNodeOrder.prototype.clear = function() {
	this.grid.clear();
}
...
TreeNodeOrderGrid.prototype.clear = function() {
	var childs = this.tBody.childNodes; 
	for(var i = 0; i < childs.length; i++) { 
		this.tBody.removeChild(childs[i]); 
	}
	this.selectedRowList = new Array();
}
